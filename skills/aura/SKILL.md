...
```
Step 1: PM-Discovery (model: haiku, context:fork)
        → Opportunity Solution Tree (OST)
        → 고객 페인포인트, 원하는 결과, 실험 목록
        출력: .aura/pm/discovery.md

Step 2: PM-Strategy (model: haiku, context:fork)
        → JTBD 6-Part 분석
        → Lean Canvas (핵심 항목만)
        출력: .aura/pm/strategy.md

Step 3: PM-PRD (model: sonnet, context:fork)
        → 8섹션 PRD 자동 생성
        출력: .aura/pm/prd.md

Step 4: 사용자 확인 → /aura build: 자동 전환 (선택)
```

**3에이전트 병렬 실행** (Discovery + Strategy 동시, PRD는 둘 완료 후):
```
[Discovery] ─┐
             ├→ [PRD 생성]
[Strategy]  ─┘
...
```
💰 [티어: ECO/PRO/MAX] | 잔여 컨텍스트: [Y]% | 다음: [후속 제안]
```
...
*AuraKit v3.1 — Discovery-First · 5중 보안 · Tiered Model ~55% 절감 · 8개 언어 56개 명령 · ITERATE/TDD/PM 모드 · 12에이전트 · ECO/PRO/MAX 티어*

...
| **STYLE** | `style:` | 스타일, 출력 모드, learning, expert, concise, 응답 형식 |
| **SNIPPETS** | `snippets:` | 스니펫, 프롬프트 저장, 재사용, 단축어, library |
...
## G7. STYLE / SNIPPETS 모드 (SuperPower 벤치마킹)

> STYLE 상세 → `resources/style-guide.md`
> SNIPPETS 상세 → `resources/snippets-guide.md`

### STYLE 모드 — 출력 페르소나 전환

```
/aura style:learning    → 학습자 모드 (코드 주석 풍부, 이유 설명, 용어 설명)
/aura style:expert      → 전문가 모드 기본값 (핵심만, 파일:줄번호 참조)
/aura style:concise     → 초간결 모드 (최대 3줄)

# 모드와 함께 사용
/aura style:learning build:로그인 기능
/아우라 style:concise 버그 수정해줘
```

**영구 설정**: `.aura/project-profile.md`의 `Output Style: [스타일]` 필드.
**💰 라인 표시**: 기본(expert)과 다를 때만 표시: `💰 ECO | learning 모드 | ...`

### SNIPPETS 모드 — 로컬 프롬프트 라이브러리

```
/aura snippets:list              → 저장된 스니펫 목록
/aura snippets:use [이름]        → 스니펫 불러와서 실행
/aura snippets:add [이름]        → 현재 요청을 스니펫으로 저장
/아우라 snippets:delete [이름]   → 스니펫 삭제
```

**내장 스니펫**: `auth-jwt`, `crud-api`, `component-split`, `security-audit`, `env-setup`, `test-add`

**변수 치환**: `--var KEY=VALUE` 옵션으로 스니펫 내 `{{VARIABLE}}` 치환.

```bash
/aura snippets:use crud-api --var RESOURCE=products
```

**저장 위치**: `.aura/snippets/*.md` (git 포함 가능, 팀 공유)
...
*AuraKit v3.4 — Discovery-First · 5중 보안 · Tiered Model ~55% 절감 · 8개 언어 56개 명령 · ITERATE/TDD/PM/PLAN/DESIGN/REPORT/PIPELINE/STYLE/SNIPPETS 모드 · 15에이전트 · ECO/PRO/MAX 티어 · 토큰 추적 · REVIEW→Gap→ITERATE→/simplify 자동 체인*
